...
<div class="row">
 ...
 <?php for($i=1;$i<=$ctn;$i++): ?>
     <tr>     
        <?php if($i==4) { ?>     
    <td><a><?php echo $form->dropDownList($model,"[$i]item_id",CHtml::listData(Item::model()->findAllByAttributes(array('item_id'=>3)), 'item_id', 'item_name'),array('class'=>'form-control','readonly'=>true)); ?>
    <?php echo $form->error($model,'item_id'); ?></a></td>
  <?php } else {   ?>
  <td><a>
  <?php echo $form->dropDownList($model,"[$i]item_id",CHtml::listData(Item::model()->findAllByAttributes(array('item_id'=>$i)), 'item_id', 'item_name'),array('class'=>'form-control','readonly'=>true));   ?></a></td>
