<?php
/* @var $this StudentKitController */
/* @var $model StudentKit */

$this->breadcrumbs=array(
  'Student Kits'=>array('index'),
  'Manage',
);

$this->menu=array(
  array('label'=>'List StudentKit', 'url'=>array('index')),
  array('label'=>'Create StudentKit', 'url'=>array('create')),
);

Yii::app()->clientScript->registerScript('search', "
$('.search-button').click(function(){
  $('.search-form').toggle();
  return false;
});
$('.search-form form').submit(function(){
  $('#student-kit-grid').yiiGridView('update', {
    data: $(this).serialize()
  });
  return false;
});
");
?>

<h1>Manage Student Kits</h1>
</section>
<section class="content">
<!--<p>
You may optionally enter a comparison operator (<b>&lt;</b>, <b>&lt;=</b>, <b>&gt;</b>, <b>&gt;=</b>, <b>&lt;&gt;</b>
or <b>=</b>) at the beginning of each of your search values to specify how the comparison should be done.
</p>

<?php echo CHtml::link('Advanced Search','#',array('class'=>'search-button')); ?>
<div class="search-form" style="display:none">
<?php //$this->renderPartial('_search',array('model'=>$model,)); ?>
</div> search-form -->

<a href="<?php echo Yii::app()->request->baseUrl; ?>index.php?r=studentJersy/create" class="btn btn-primary pull-right"> Assign Jersy >> </a><br/><br/>
<div class="row">
   <div class="col-xs-12">
    <div class="box">
     <div class="box-header">
     </div><!-- /.box-header -->
    <div class="box-body table-responsive">
<?php $this->widget('zii.widgets.grid.CGridView', array(
  'id'=>'student-kit-grid',
  'itemsCssClass' => 'table table-bordered table-striped',
  'dataProvider'=>$model->search(),
  'filter'=>$model,
  'columns'=>array(
    'id',
    array(      
            'header'=>'Student Name',    
                      'name'=>'student_id',
                      //'value'=>'$data->operator->operator_name',
            'value'=>'Registration::model()->findByPk((int)$data->student_id)->name',
                      ),
    array(
                      'name'=>'item_id',
                      'value'=>'$data->item->item_name',
                      ),
    array(
                      'name'=>'brand_id',
                      'value'=>'$data->brand->brand_name',
                      ),
    array(
                      'name'=>'color_id',
                      'value'=>'$data->color->color_name',
                      ),
    array(
                      'name'=>'size_id',
                      'value'=>'$data->size->size',
                      ),
    'status',
    'replacement_fees',
    /*
    'inventory_id',
    'date',
    
    */
    array(
      'class'=>'CButtonColumn',
      'template'=>'{view}{update}{delete}',
      'buttons'=>array(
            'update' => array(
        'url'=>'Yii::app()->createUrl("studentKit/update", array("id"=>$data->student_id))'
    ),
    ),
    ),
  ),
)); ?>
</div><!-- /.box-body -->
  </div><!-- /.box -->
    </div>
      </div>