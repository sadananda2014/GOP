...
class UserModule extends CWebModule
{
	...
	static private $_coach;
	static private $_coachs;
	static private $_dataentry;
	static private $_dataentrys;
    ...
    if(Yii::app()->user->isGuest)
      ...
      if (!isset(self::$_coach)) {
        if(Profile::model()->findbyPk(Yii::app()->user->id)->roles=="1")
          self::$_coach = true;
        else
          self::$_coach = false;  
      }
  ...
  public static function isDataentry() {
    if(Yii::app()->user->isGuest)
      ...
      if (!isset(self::$_dataentry)) {
        if(Profile::model()->findbyPk(Yii::app()->user->id)->roles=="2")
          self::$_dataentry = true;
        else
          self::$_dataentry = false;  
      }
      return self::$_dataentry;
    }
  }
	...
	public static function getCoachs() {
    if (!self::$_coachs) {
      $Criteria = new CDbCriteria();
               $Criteria->condition = "roles='1'";
               $Criteria->join =' JOIN (tbl_profiles) ON (t.id=tbl_profiles.user_id)';
               //this function returns the list of categories to use in a dropdown          

      $coachs = User::model()->findAll($Criteria);
      $return_name = array();
      foreach ($coachs as $coach)
        array_push($return_name,$coach->username);
      self::$_coachs = ($return_name)?$return_name:array('');
    }
    return self::$_coachs;
  }
  
  
  public static function getDataentrys() { 
    if (!self::$_dataentrys) {
      $Criteria = new CDbCriteria();
			   $Criteria->join =' JOIN (tbl_profiles) ON (t.id=tbl_profiles.user_id)';
               $Criteria->condition = "roles='2'";
               
               //this function returns the list of categories to use in a dropdown          

      $dataentrys = User::model()->findAll($Criteria);
      $return_name = array();
      foreach ($dataentrys as $dataentry)
        array_push($return_name,$dataentry->username);
      self::$_dataentrys = ($return_name)?$return_name:array('');
    }
    return self::$_dataentrys;
  }
